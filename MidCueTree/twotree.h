...
public:
    ...
    {  data = e;  left=L; right=R; havel = 1; haver = 1;};
};
...
void BTree<elemType>::BecomeCue() {
    ...
    while(!que.isEmpty()){
        ...
        while(!que.isEmpty()){
            ...
            if(!n->right){
                if (!que.isEmpty()) n->right = que.front();

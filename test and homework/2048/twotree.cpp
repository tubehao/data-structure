...
public:
    ...
    void deleteTree(node<elemType> *t);
...
template<class elemType>
void BTree<elemType>::deleteTree(node<elemType> *p)
{
    if(p == nullptr) return;
    if(p->left!= nullptr) {
        deleteTree(p->left);
...
bool cut(node<int> *now){
    bool leftcut= false,rightcut= false;
    if(now->left != nullptr) leftcut = cut(now->left);
    if(now->right != nullptr) rightcut = cut(now->right);
    if(leftcut == 1) now->left = nullptr;
    if(rightcut == 1) now->right = nullptr;
    if(now->left == nullptr && now->right == nullptr && now->data == 0) {
        delete now;
        return true;
    }
    return false;
